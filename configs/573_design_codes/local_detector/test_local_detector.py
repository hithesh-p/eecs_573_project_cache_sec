...
def test_CRICMILocalDetector():
    ...
    #simulating a single bucket for simplicity now
    #we can use diff buckets for diff domains
    detector = CRICMILocalDetector(threshold=3, num_buckets=1, interval_limit=10)
    ...
    access_patterns = [
        ...
        (1, 1, 1),
        (1, 1, 2),
        (1, 1, 3),  # This should trigger an event due to cyclic interference
        (3, 3, 1),  # Another interference
        (2, 2, 3),  # Another interference, should reach threshold here and trigger an alert
    ]
    ...
    for i, (previous_domain, request_domain, current_domain) in enumerate(access_patterns):
        ...
        print(f"Event Counter (8-bit): {detector.event_counter}")
